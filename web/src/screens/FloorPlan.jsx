...
function ObjectsPanel({ ctx, plan }) {
  ...
  const [form, setForm] = useState({ presetKey: "", layer: "walls", kind: "wall", label: "", room_id: "", product_id: "", material_id: "", geometry: {} });
  ...
  function pickLayer(layer) {
    setForm((p) => ({ ...p, presetKey: "", layer, kind: KIND_BY_LAYER[layer][0], room_id: layer === "annotations" ? p.room_id : "", geometry: {} }));
  }
  function applyPreset(key) {
    ...
    setForm((p) => ({
      ...
      layer: preset.layer,
      kind: preset.kind,
      room_id: preset.layer === "annotations" ? p.room_id : "",
  ...
  async function add() {
    ...
    try {
      const geometry = {
        ...(form.geometry || {}),
        ...(form.layer === "annotations" && form.room_id ? { room_id: form.room_id } : {})
      };
      ...
      setForm((p) => ({ ...p, presetKey: "", label: "", room_id: "", product_id: "", material_id: "", geometry: {} }));
      await load();
