...
Verantwoordelijkheden: plattegrondrecords, uploads, eenvoudige tekening-JSON met gelabelde maatlijnen (`drawing_json.dimensions`), **schaal** (`scale_ratio`/`scale_unit`), **vector-objecten op lagen** (`floorplan_objects`: walls/meubels/annotaties, CRUD per object), **product/materiaal-koppeling per object** (`floorplan_objects.product_id` / `material_id` met FK ON DELETE SET NULL; de objects-GET joint `products`/`materials` voor `product_name`/`material_name`), **versiebeheer** (`/new-version` kloont plattegrond + objecten incl. koppelingen) en thumbnails.
