...
Werkt: floorplan aanmaken met upload of SVG-tekening, bewerken na aanmaak, verwijderen. **Schaal** (`scale_ratio`/`scale_unit`), **vector-objecten** op **lagen** (`floorplan_objects`: walls/meubels/annotaties, CRUD per object; annotaties kunnen per ruimte worden gescoped via `geometry_json.room_id`), **product/materiaal-koppeling** per object (`product_id`/`material_id`, FK met ON DELETE SET NULL; gekoppelde naam wordt geresolveerd in de objects-GET), **versiebeheer** (`/:id/new-version` kloont de plattegrond + objecten incl. koppelingen), thumbnail-veld.
