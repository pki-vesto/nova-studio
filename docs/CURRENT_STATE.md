...
Werkt: floorplan aanmaken met upload of SVG-tekening, bewerken na aanmaak, verwijderen. **Schaal** (`scale_ratio`/`scale_unit`), **vector-objecten** op **lagen** (`floorplan_objects`: walls/meubels/annotaties, CRUD per object) met herbruikbare objectpresets voor meubels/wanden/annotaties, **product/materiaal-koppeling** per object (`product_id`/`material_id`, FK met ON DELETE SET NULL; gekoppelde naam wordt geresolveerd in de objects-GET), **versiebeheer** (`/:id/new-version` kloont de plattegrond + objecten incl. koppelingen), thumbnail-veld.
